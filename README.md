...
- *견인 모드에서 주행 불안정(피시테일) 발생**
  ...
  - 해결: 경로 상 2개 예측점으로 목표 진행방향(heading)을 atan2 기반으로 계산하고, 목표 heading과 현재 yaw의 오차가 임계각을 초과하면 선속도를 제한한 뒤 각속도 우선으로 heading을 먼저 정렬하고, 정렬 완료 후 경로 추종을 재개하도록 제어 로직을 분기함.
